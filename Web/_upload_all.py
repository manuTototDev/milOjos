"""
Upload images to HF Space - resilient version.
Waits 10s between batches and up to 5 min on rate limits.
Run this in background and let it work.
"""
import os, time, sys
    ...
    for i in range(0, len(to_upload), BATCH):
        ...
        for attempt in range(10):
            try:
                ...
                el = time.time()-t0
                eta = (len(to_upload)-i-len(batch)) / (done/el) if done else 0
                print(f"  [{done}/{len(to_upload)}] {el:.0f}s, ETA {eta:.0f}s")
                break
            except Exception as e:
                wait = min(30 * (attempt+1), 300)
                print(f"  retry {attempt+1}, wait {wait}s: {str(e)[:60]}")
                time.sleep(wait)
...
# Wait before starting if fresh from rate limit
print(f"Starting upload at {time.strftime('%H:%M:%S')}")
print(f"Waiting 5 min for rate limit cooldown...")
time.sleep(300)
